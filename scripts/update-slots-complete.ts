...
function parseSlotCode(slotCode: string): { code: string, occ: string } {
  // Handle LAB slots
  if (slotCode.startsWith('LAB-')) {
    return { code: 'LAB', occ: slotCode.substring(4) }
  }
  
  // Handle slots with parentheses like A3(1), A3(1, 2)
  const match = slotCode.match(/^([A-Z]\d+)(\(.*\))?$/)
  if (match) {
    const baseCode = match[1]
    const occPart = match[2] || ''
    // Remove parentheses and spaces from occ
    const occ = occPart.replace(/[\(\)\s]/g, '')
    return { code: baseCode, occ: occ || '1' }
  }
...
// Convert occ string to number for database
function occToNumber(occStr: string): number {
  // If empty, return 1
  if (!occStr) return 1
  // If it's a number, parse it
  if (/^\d+$/.test(occStr)) return parseInt(occStr)
  // If it contains comma (like "1,2"), return 12 to make it unique
  if (occStr.includes(',')) {
    return parseInt(occStr.replace(/,/g, ''))
  }
  // For special cases like "4h", return 99
  if (occStr === '4h') return 99
  // For lab clusters (single letters), use ASCII code
  if (/^[A-Z]$/.test(occStr)) return occStr.charCodeAt(0)
  // Default
  return 1
}
...
const formattedSlots = slotData.map(slot => {
  ...
  return {
    ...
    // Store the original slot code in a custom field for display
    display_code: slot.slot_code
  }
})
...
async function updateSlots() {
  ...
  try {
    ...
    console.log(`\nVerification: Database now contains slots`)
    console.log('\nSample of inserted slots:')
    verifyData?.forEach(slot => {
      const displayCode = slot.code + (slot.occ !== 1 ? `(${slot.occ})` : '')
      console.log(`${displayCode} - ${slot.day || 'N/A'} ${slot.start_time} to ${slot.end_time} ${slot.is_lab ? '(LAB)' : ''} ${slot.cluster ? `[${slot.cluster}]` : ''}`)
    })
